...
import type { AgentEvent } from '../src/pi/types';
import {
  assistantMessage,
  attrs,
  CapturingExporter,
...
// several of its own local instrumentPiCodingAgent() calls (not
// pi-test-helpers.ts's makeRig()) because those tests need custom Session
// classes (streaming-aware, command-aware, input-hook-aware) that makeRig()
// doesn't support.

 ...
 * complete pair at boundary), config baseUrl whitespace normalization,
 * circular-ref handling through openToolSpan, and provider forceFlush
 * idempotency — surfaces the rest of the suite leaves thin.
 */
...
import type { AgentMessage, AssistantMessage } from '../src/types';
...
// --- config.ts: whitespace normalization --------------------------------
...
test('a whitespace-only baseUrl falls back to the default rather than becoming a broken exporter URL', () => {
  withEnv({ TRACEROOT_HOST_URL: undefined }, () => {
    const resolved = resolveConfig({ apiKey: 'k', baseUrl: '   ' });
    // Same failure mode the "" and "///" guards exist to prevent: a
    // whitespace-only value spliced into `${baseUrl}/api/v1/public/traces`
    // yields a broken URL that silently drops every span.
    assert.equal(resolved.baseUrl, 'https://app.traceroot.ai');
  });
});

// --- provider.ts: forceFlush -------------------------------------------

test('a private-mode forceFlush resolves and is safe to call repeatedly', async () => {
  const spans: ReadableSpan[] = [];
  const exporter = {
    export(batch: ReadableSpan[], cb: (r: { code: number }) => void) {
      spans.push(...batch);
      cb({ code: 0 });
    },
    async shutdown() {},
  };
  const handle = createTracing(
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    resolveConfig({ apiKey: 'k', _spanExporter: exporter as any }),
  );
  assert.equal(handle.ownsProvider, true);
  await assert.doesNotReject(handle.forceFlush());
  await assert.doesNotReject(handle.forceFlush());
});
